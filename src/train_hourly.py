...
from util import *
from load_hourly import load_wind, load_solar_data, load_wind_data_spatial #Change the data source for other tasks
...
print("Shape of label samples ", shape(trY))
...
print([var for var in tf.trainable_variables() if var.name.startswith('gen')])
print("shape of train_op_discrim",discrim_vars)
print("shape of train_op_gen",gen_vars)
...
for epoch in range(n_epochs):
    ...
    for start, end in zip(
        ...
        Xs = trX[start:end].reshape([-1, 5, 5, 1])
        Ys = trY2[start:end]
        ...
        if np.mod(iterations, 5000) == 0:
            ...
            print("Discrim loss:", discrim_loss_val)
            Y_np_sample = OneHot(np.random.randint(2, size=[visualize_dim]), n=events_num)
            ...
            generated_samples=generated_samples.reshape([-1,25])
            generated_samples = generated_samples * 1 # did not use normalized data, keep the value unchanged
