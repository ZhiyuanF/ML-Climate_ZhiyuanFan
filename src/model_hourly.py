...
class GAN():
    def __init__(
            ...
            image_shape=[5,5,1],
            dim_z=100,
            dim_y=2, #The parameters for controlling the number of events
            dim_W1=1024,
            dim_W2=128,
            dim_W3=64,
            dim_channel=1,
        ...
        self.gen_W2 = tf.Variable(tf.random.normal([dim_W1+dim_y, dim_W2*2*2], stddev=0.02), name='gen_W2')
        self.gen_W3 = tf.Variable(tf.random.normal([2,2,dim_W3,dim_W2+dim_y], stddev=0.02), name='gen_W3')
        self.gen_W4 = tf.Variable(tf.random.normal([2,2,dim_channel,dim_W3+dim_y], stddev=0.02), name='gen_W4')

        self.discrim_W1 = tf.Variable(tf.random.normal([2,2,dim_channel+dim_y,dim_W3], stddev=0.02), name='discrim_W1')
        self.discrim_W2 = tf.Variable(tf.random.normal([1,1,dim_W3+dim_y,dim_W2], stddev=0.02), name='discrim_W2')
        print("discrim_W2 shape defined with", self.discrim_W2.get_shape())
        self.discrim_W3 = tf.Variable(tf.random.normal([dim_W2*1*1+dim_y,dim_W1], stddev=0.02), name='discrim_W3')
        print("discrim_W3 shape defined with", self.discrim_W3.get_shape())
    ...
    def build_model(self):
        ...
        print("discrim_cost is {}".format(discrim_cost))
        print("gen_cost is {}".format(gen_cost))
    ...
    def discriminate(self, image, Y):
        ...
        X = tf.concat([image, yb * tf.ones([self.batch_size, 5, 5, self.dim_y])],3)
        print("X shape", X.get_shape())
        h1 = lrelu( tf.nn.conv2d( X, self.discrim_W1, strides=[1,2,2,1], padding='SAME' ))
        print("h1 shape", h1.get_shape())
        h1 = tf.concat([h1, yb * tf.ones([self.batch_size, 3, 3, self.dim_y])],3)
        print("h1 shape", h1.get_shape())
        h2 = lrelu(batchnormalize( tf.nn.conv2d( h1, self.discrim_W2, strides=[1,3,3,1], padding='SAME')) )
        print("h2 shape", h2.get_shape())
        h2 = tf.reshape(h2, [self.batch_size, -1])
        h2 = tf.concat([h2, Y], 1)
        print("h2 shape (after concat)", h2.get_shape())
        print("discrim_W3 shape", self.discrim_W3.get_shape())
    ...
    def generate(self, Z, Y):
        ...
        print("h2 shape", h2.get_shape())
        h2 = tf.reshape(h2, [self.batch_size,2,2,self.dim_W2])
        print("h2 shape", h2.get_shape())
        h2 = tf.concat([h2, yb*tf.ones([self.batch_size, 2,2, self.dim_y])],3)
        # n=yb*tf.ones([self.batch_size, 6,6, self.dim_y])
        # print("shape of yb new",n.get_shape() )
        print("h2 shape", h2.get_shape())

        output_shape_l3 = [self.batch_size,3,3,self.dim_W3]
        ...
        h3 = tf.concat([h3, yb*tf.ones([self.batch_size, 3, 3, self.dim_y])], 3)
        print("h3 shape", h3.get_shape())

        output_shape_l4 = [self.batch_size,5,5,self.dim_channel]
    ...
    def samples_generator(self, batch_size):
        ...
        h1 = tf.concat([h1, Y], 1)
        h2 = tf.nn.relu(batchnormalize(tf.matmul(h1, self.gen_W2)))
        h2 = tf.reshape(h2, [batch_size,2,2,self.dim_W2])
        h2 = tf.concat([h2, yb*tf.ones([batch_size, 2,2, self.dim_y])], 3)

        output_shape_l3 = [batch_size,3,3,self.dim_W3]
        h3 = tf.nn.conv2d_transpose(h2, self.gen_W3, output_shape=output_shape_l3, strides=[1,2,2,1], padding="SAME")
        h3 = tf.nn.relu( batchnormalize(h3) )
        h3 = tf.concat([h3, yb*tf.ones([batch_size, 3,3,self.dim_y])], 3)

        output_shape_l4 = [batch_size,5,5,self.dim_channel]
        h4 = tf.nn.conv2d_transpose(h3, self.gen_W4, output_shape=output_shape_l4, strides=[1,2,2,1], padding="SAME")
        x = tf.nn.sigmoid(h4)
        return Z, Y, x
